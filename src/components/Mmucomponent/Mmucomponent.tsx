...
interface PageTableEntry {
  frame: number;
  valid: boolean;
  originalFrame: number | null; // Novo campo para armazenar o quadro original
}
...
const unitSize = 4; // Tamanho da unidade em bytes (4 bytes por unidade)
const initialMemorySize = 3; // Inicialmente 3 quadros de memória de 1024 bytes cada
const maxPagesAndFrames = 6; // Limite máximo de páginas e quadros
...
const MmuComponent: React.FC = () => {
  const [pageTable, setPageTable] = useState<PageTableEntry[]>([
    ...
    { frame: 0, valid: true, originalFrame: 0 }, // Página 2 como válida para simulação
  ]);
  ...
  const calculatePhysicalAddress = () => {
    ...
    if (entry.valid) {
      const address = entry.frame * 1024 + offset * unitSize; // Multiplicando o offset pelo tamanho da unidade
      setPhysicalAddress(address.toString(16).toUpperCase()); // Convertendo o endereço físico para hexadecimal
  ...
  const togglePageValidity = (index: number) => {
    setPageTable((prevPageTable) =>
      prevPageTable.map((page, i) => {
        if (i === index) {
          return page.valid
            ? { ...page, valid: false } // Tornar a página inválida
            : { ...page, valid: true, frame: page.originalFrame! }; // Restaurar a página com o quadro original
        }
        return page;
      })
    );
