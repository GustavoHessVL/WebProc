...
const ARMEmulator: React.FC = () => {
  ...
  const [pcBeforeMov, setPcBeforeMov] = useState<string>(registers.r15);
  ...
  const handleMOV = (dest: string, src: string) => {
    ...
    setRegisters((prevRegisters) => {
      ...
      if (dest === 'r15') {
        newRegisters.r15 = prevRegisters[src];
      }
  ...
  return (
    <div>
      ...
      <div style={{ marginTop: '20px' }}>
        <h3>PC Before MOV: {pcBeforeMov}</h3>
        <h3>PC After MOV: {registers.r15}</h3>
      </div>
    </div>
  );
};
