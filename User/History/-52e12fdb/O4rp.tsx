...
const ToggleButton4 = () => {
  ...
  const calculateExecution = (condState : number[], cpsrState: number[]) => {
    ...
    switch(condKey){
      ...
      case '1000':
        // Precisa justificar !!!!!!!!
        if(cpsrState[2] == 0 && cpsrState[1] == 1){
          return "Executa instrução! Carry flag indica operação excedendo e Zero flag não nula ";
        }
        return "Não executa instrução!";
      case '1001':
        // PRECISA JUSTIFICAR!!!!!!!!!!!!!!
        if(cpsrState[1] == 0){
          return "Executa instrução! Carry flag inativa indica que não é maior!"
        }
        else if(cpsrState[2] == 1){
          return "Executa instrução! Zero flag ativa indica que é igual!"
        }
        return "Não executa instrução!";
      case '1010':
        if(cpsrState[3] == 0 && cpsrState[0] == 0){
          return "Executa instrução! Negativo inativo e sem Overflow indica não ser negativo! Logo, ser maior ou igual!"
        }
        else if(cpsrState[3] == 1 && cpsrState[1] == 1){
          // NECESSITA DE JUSTIFICATIVA
          return "Executa instrução! Negativo ativo e Overflow ativo!"
        }
        return "Não executa instrução!";
      case '1011':
        if(cpsrState[3] == 0 && cpsrState[0] == 1){
          // NECESSITA DE JUSTIFICATIVA
          return "Executa instrução!"
        }
        else if(cpsrState[3] == 1 && cpsrState[1] == 0){
          // NECESSITA DE JUSTIFICATIVA
          return "Executa instrução! Flag negativa ativa indica que na operação de comparação o número era menor!"
        }
        return "Não executa instrução!";
      case '1100':
        if(cpsrState[3] == 0 && cpsrState[2] == 0 && cpsrState[0] == 0 ){
          return "Executa instrução! Negative Flag em baixa indica número não negativo, Zero Flag em baixa indica número não nulo e Overflow indica que a operação ocorreu respeitando o tamanho do resultado!"
        }
        if(cpsrState[3] == 1 && cpsrState[2] == 0 && cpsrState[0] == 0){
          // Precisa justificar, não lembro
          return "Executa Instrução!";
        }
        return "Não executa instrução!"
      case '1101':
        if (cpsrState[1] == 1) {
          ...
          // Não sei justificar tem que ver!!!!!
          return "Executa instrução!"
        }
        return "Não executa instrução!";
      case '1110':
