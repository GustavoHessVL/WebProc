...
const AritComponent: React.FC = () => {
    ...
    const calculateExecutionText = (opcode:string,s:string,Rn:string,Rd:string,shifter:string,tipo:string,R:string,Rm:string) => {
        ...
        if(getOpcodeText(opcode) == 'cmp'){
            result = result.concat(' Note que esta é uma instrução de comparação, equivalente a um subs, entretanto, não salvando o valor no registrador de destino.');
            return result;
        }

        if(getOpcodeText(opcode) == 'cmn'){
            result = result.concat(' Note que esta é uma instrução de comparação, equivalente a um adds, entretanto, não salvando o valor no registrador de destino.');
            return result;
        }

        if(getOpcodeText(opcode) == 'tst'){
            result = result.concat(' Note que esta é uma instrução de teste lógico, equivalente a um ands, entretanto, não salvando o valor no registrador de destino. ')
            return result;
        }

        if(getOpcodeText(opcode) == 'teq'){
            result = result.concat(' Note que esta é uma instrução de teste lógico, equivalente a eors, entretanto, não salvando o valor no registrador de destino. Percebe-se que com essa instrução verifica-se a igualdade sem mexer nas flags C e V.')
            return result;
        }
        ...
        if(getOpcodeText(opcode) == 'bic'){
            result = result.concat(' Note que esta é uma instrução de bit clear, portanto, zera bits do registrador de destino.')
        }


        if(getOpcodeText(opcode) == 'mov'){
            result = result.concat(' Note que esta é uma instrução de movimentação de dados, assim, opera movimentando de um registrador ao outro o valor. ')
            return result;
        }

        if(getOpcodeText(opcode) == 'mvn'){
            result = result.concat(' Note que esta é uma instrução de movimentação de dados, assim, opera movimentando de um registrador ao outro o valor, nesse caso, invertendo os bits. ')
            return result;
        }
