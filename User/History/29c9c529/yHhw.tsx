...
const AritComponent: React.FC = () => {
    ...
    const calculateExecutionText = (opcode:string,s:string,Rn:string,Rd:string,shifter:string,tipo:string,R:string,Rm:string) => {
        ...
        if(R != 'x'){
            if(R == '1'){
                if(shifter != 'xxxxx'){
                    result = result.concat(' Sobre o segundo operador há um shift, como o bit R está ativo, é um shift de acordo com o valor do registrador dos quatro primeiros bits do shifter : ' + shifter.slice(0,4)) + '.';
                }
            }
            else{
